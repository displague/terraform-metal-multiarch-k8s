...
for cert in {sa,{front-proxy-,etcd/,}ca}.{key,crt} 
    /usr/bin/scp -3 -i $ssh_private_key_path -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -q root@$controller:/etc/kubernetes/pki/$cert root@$node_addr:/etc/kubernetes/pki/$cert
done
echo "waiting..."
sleep 360
